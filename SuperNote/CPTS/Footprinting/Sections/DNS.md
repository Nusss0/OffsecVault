...
![[Pasted image 20260902074430.png]]
...
> [!example]- SOA Usage 
> ```shell
> nusss@htb[/htb]$ dig soa www.inlanefreight.com
>
...
> [!Example]- Example Of Local DNS Configuration
> ```shell
...
> [!info]- Some Dangerous Setting Configuration
>
>|**Option**|**Description**|
|---|---|
...
> [!example]- Using Dig
> ```shell
> nusss@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
>
...
> [!example]- DIG - Query Version
> ```shell
>nusss@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
>
...
> [!example]- DIG - Any Queries
> ```shell
> nusss@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
>
...
---
## Zone Transfer
> *Males rangkum* Access at : [HTB Acads](https://academy.hackthebox.com/app/module/112/section/1069)
...
> [!example]- DIG - AXFR Zone Transfer
> ```shell
> nusss@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
>
...
> [!example]- DIG - AXFR Zone Transfer - Internal
> ```shell
> nusss@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
>
...
> We can use a list that have been provided by [SecLists](https://github.com/danielmiessler/SecLists/blob/master/Discovery/DNS/subdomains-top1million-5000.txt) to send request in order.

> [!example]- Subdomain Brute Forcing
> ```shell
> nusss@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
>
ns.inlanefreight.htb.   604800  IN      A       10.129.34.136
mail1.inlanefreight.htb. 604800 IN      A       10.129.18.201
app.inlanefreight.htb.  604800  IN      A       10.129.18.15
...
Another tool to do bruteforce is [DNSenum](https://github.com/fwaeytens/dnsenum).

> [!example]- DNSenum 
> ```shell
> nusss@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
>
